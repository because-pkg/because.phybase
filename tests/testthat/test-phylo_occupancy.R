...
for (s in 1:N_species) {
  logit_p <- alpha_p[s] + beta_p[s] * Trait[s] +
    gamma_p * Trait[s] * Habitat
...
res <- because(
  equations   = eqs,
  data        = list(Y = Y_obs,          # list of detection matrices (species dimension)
    Habitat = Habitat,
    Trait   = Trait),
...
summ <- summary(res)
alpha_psi_est <- summ$results[grep("^alpha_Y", rownames(summ$results)), ]
...
plot(alpha_psi, alpha_psi_est$Mean,
